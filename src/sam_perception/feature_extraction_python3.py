...
class AdaptiveThreshold:
    ...
    def process(self, img):
        ...
        contours, _, _ = cv.findContours(imgTemp, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)

        while len(contours) < self.nFeatures and self.threshold >= self.thresMin+self.thresInc:
            self.threshold -= self.thresInc
            ret, imgTemp = cv.threshold(img, self.threshold, 256, self.thresholdType)
            contours, _, _ = cv.findContours(imgTemp, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
            # find_countours_result = cv.findContours(imgTemp, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
            # contours, hier = find_countours_result[0], find_countours_result[1]
...
class AdaptiveErodeKernel:
    ...
    def process(self, img):
        # this removes reflections effectively at close distances but reduces range
        contours, _, _ = cv.findContours(img, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
        contours.sort(key=contourRatio, reverse=True)
...
def findContourAt(gray, center):
    contours, _, _ = cv.findContours(gray, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
...
def findPeakContourAt(gray, center, offset=None, mode=cv.RETR_EXTERNAL):
    contours, _, _ = cv.findContours(gray, mode, cv.CHAIN_APPROX_SIMPLE)


    if offset is None:
        contoursOffset = [cnt for cnt in contours]
    else:
        contoursOffset, _, _ = cv.findContours(gray, mode, cv.CHAIN_APPROX_SIMPLE, offset=offset)
...
def findNLocalMax(gray, kernel, p, n, margin=1, offset=(0,0), drawImg=None, drawInvalidPeaks=False):
    ...
    if offset is None:
        ...
        contoursOffset, _, _ = cv.findContours(gray, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE, offset=offset)
...
class ThresholdFeatureExtractor:
    ...
    def __call__(self, gray, drawImg=None):
        ...
        contoursNew, _, _ = cv.findContours(img, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)
...
class AdaptiveThreshold2:
    ...
    def process(self, gray, maxAdditionalCandidates=0, estDSPose=None, roiMargin=None, drawImg=None):
        ...
        contours, _, _ = cv.findContours(imgTemp, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
